...
const AIConsultant: React.FC<AIConsultantProps> = ({ onSelectPackage }) => {
  ...
  return (
    <div className="bg-[#0c0e14] border border-emerald-500/20 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row min-h-[500px]">
      <div className="md:w-1/3 bg-gradient-to-br from-emerald-900/40 to-black p-8 flex flex-col justify-between">
        <div>
          <h2 className="text-3xl font-extrabold mb-4">Smart Detailing <span className="text-emerald-400">Advisor</span></h2>
          ...
          <ul className="space-y-4">
            <li className="flex items-center text-xs text-emerald-300 bg-emerald-500/10 p-2 rounded-lg border border-emerald-500/20">
          ...
          <div className="flex flex-wrap gap-2">
            {['Muddy', 'Pet Hair', 'Old Wax', 'Spills'].map(tag => (
              <button 
                ...
                className="text-[10px] px-2 py-1 bg-white/5 rounded hover:bg-white/10 transition-colors text-emerald-400"
              >
                #{tag}
              </button>
            ))}
        ...
        <div className="p-4 border-t border-white/5 bg-[#0a0c10]">
          <div className="flex space-x-2">
            <input 
              ...
              className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 transition-colors text-white"
            />
            <button 
              onClick={handleSend}
              disabled={isTyping}
              className="bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 px-6 rounded-xl font-bold transition-all text-white"
            >
              Send
            </button>
          </div>
